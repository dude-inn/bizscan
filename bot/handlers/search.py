...
async def ask_inn(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(search_type="inn")
...
async def _show_company_choices(message_or_cb, companies: list, state: FSMContext, page: int = 0):
    ...
    if len(companies) == 1:
        company = companies[0]
        # Извлекаем ИНН из результата поиска
        ...
        name_full = company.get("НаимПолн") or company.get("name_full") or company.get("full_name") or company.get("name") or "Неизвестно"
        await message_or_cb.answer(
            f"✅ Найдено: {name_full} — ИНН {inn}\nВыберите тип отчёта:",
